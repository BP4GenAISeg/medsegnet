...
class WandBLogger:
    ...
    def log_metrics(self, metrics: dict, step: int | None = None, commit: bool = False):
        ...
        if self.run:
            self.run.log(
                metrics, step=step, commit=commit
            )  # commit=True ensures data is synced
...
def get_wandb_logger(
    cfg: DictConfig, model: torch.nn.Module | None = None
) -> WandBLogger | None:
    """
    Initializes and returns a WandBLogger instance based on config.
    Returns None if tracking is disabled in the config.
    """
    if cfg.tracking.get("log_to_wandb", False):  # Check if wandb logging is enabled
        return WandBLogger(config=cfg, model=model)
    else:
        logger.warning(
            "WandB tracking is disabled in the configuration. No logger will be created."
        )
