...
class Trainer:
    def __init__(
        ...
        self.fusion_fn = get_fusion_fn(
            unified_cfg.model.deep_supervision.inference_fusion_mode
        )
    ...
    def train_one_epoch(self, epoch: int):
        ...
        try:
            for batch_idx, (images, masks) in enumerate(progress_bar):
                ...
                loss = compute_ds_loss(
                    self.criterion, outputs, masks, current_weights, self.device
                )

                if torch.isnan(loss) or torch.isinf(loss):
                    self.logger.warning(
                        f"Loss is NaN or Inf at epoch {epoch}. Skipping batch."
                    )

                    # Optionally log this event to WandB, so we can see how often it happens
                    if self.wandb_logger:
                        self.wandb_logger.log_metrics(
                            {"train/skipped_batches": 1}, step=epoch, commit=False
                        )
                    continue
    ...
    def train(self):
        ...
        try:
            for epoch in trange(num_epochs, desc="Training Progress"):
                ...
                if self.wandb_logger:
                    self.wandb_logger.log_metrics(
                        {"epoch_duration_sec": epoch_duration}, step=epoch, commit=False
                    )
                # ------------------------

                # Step ReduceLROnPlateau scheduler if used
                if self.lr_scheduler and isinstance(
                    self.lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau
                ):
            ...
            if self.wandb_logger and self.wandb_logger.run:
                ...
                if self.early_stopper is not None:
                    self.wandb_logger.run.summary["best_val_loss"] = (
                        self.early_stopper.best_loss
                    )
                    self.wandb_logger.run.summary["best_val_dice"] = (
                        self.early_stopper.best_dice
                    )
    ...
    def validate(self, epoch: int):
        ...
        with torch.no_grad():
            try:
                for batch_idx, (images, masks) in enumerate(progress_bar):
                    ...
                    if torch.isnan(loss) or torch.isinf(loss):
                        self.logger.warning(
                            f"NaN/Inf validation loss encountered at epoch {epoch}, batch {batch_idx}. Skipping batch metrics."
                        )
                        continue

                    val_loss += loss.item()

                    fused_output = call_fusion_fn(
                        self.fusion_fn, outputs=outputs, weights=current_weights
                    )
                    dice_scores_batch_per_class = dice_coefficient_classes(
                        fused_output, masks, self.num_classes, ignore_index=ignore_index
                    )

                    dice_scores_sum_per_class += torch.stack(
                        dice_scores_batch_per_class
                    )
                    dice_per_batch_avg = torch.mean(
                        torch.stack(dice_scores_batch_per_class)
                    ).item()
                    dice_score_total += dice_per_batch_avg

                    # --- Log segmentation examples to WandB (once per epoch) ---
                    if (
                        self.wandb_logger and not logged_images and batch_idx == 0
                    ):  # Log first batch
        ...
        if self.wandb_logger:
            ...
            for class_idx, score in zip(active_classes, avg_dice_per_class):
                log_data[
                    f"val/dice_class_{self.class_labels.get(class_idx, class_idx)}"
                ] = score.item()  # Use label if available

            self.wandb_logger.log_metrics(
                log_data, step=epoch, commit=False
            )  # Commit validation metrics + images here
            # -----------------------------------------------
    ...
    def test(self):
        ...
        for class_idx, score in zip(active_classes, avg_per_class):
            self.logger.info(f"     | Class {class_idx} Dice: {score.item():.4f}")
