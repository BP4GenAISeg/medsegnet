...
class Trainer:
    def __init__(
        ...
        self.enable_deep_supervision = (
            self.architecture == "ds-unet3d"
            and cfg.architectures["ds-unet3d"].model_defaults.deep_supervision.enabled
        )
        ...
        wandb_active = (
            self.wandb_logger and self.wandb_logger.run and self.wandb_logger.is_active
        )
        if not wandb_active:
        ...
        self.output_fuser = OutputFuser(
            mode=cfg.fusion.mode, weights=cfg.fusion.weights
        )
            ...
            try:
                for batch_idx, (images, masks) in enumerate(progress_bar):
                    ...
                    dice_scores_batch_per_class = dice_coefficient_classes(
                        pred_masks, masks, self.num_classes, ignore_index=ignore_index
                    )
                    ...
                    is_first_batch_and_not_logged = (
                        self.wandb_logger is not None
                        and not logged_images
                        and batch_idx == 0
                    )
